#!/usr/bin/env python3
r"""OP2 M3 quadruplet match: dogleg exit (P2) -> undulator entrance (P3).
...
no derivation script for it. The last quad of the linear solution is strong,
k1 ~ -130 1/m^2 (~17 T/m at 40 MeV), because the 0.53 m waist sits only 0.4 m
past it.
...
under space charge (~20 mm at 1 nC); the dispersion-corrected values are
...
TARGET = (0.53, 0.0, 0.53, 0.0)

# M3: the four-quad station after the dogleg. d_in is the OP2 allocation of
# the interface drift (the dogleg's d_out = 0.3 m sits upstream of P2).
GEOM = QuadrupletGeom(lq=0.10, d_in=0.30, d_inter=0.30, d_out=0.40)
...
def main():
    ...
    apply_style()
    save(figure_e2e(match, tws, SC), FIGS, 'fig_OP2_m3_quadruplet_match')
    plt.show()
