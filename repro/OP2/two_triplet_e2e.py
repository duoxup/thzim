#!/usr/bin/env python3
r"""OP2 upstream match — step 2 of 2: run the chosen working point end to end.
...
the gradients removes the ambiguity. Four panels against s, x blue and y red:
  ...
  (b) beta, with the dogleg entrance target marked; the title carries Bmag,
      ...
      filaments)
  (c) rms size, round across the shaded middle by construction. The first
      feature is NOT this section's doing: the OP2 P0 beam arrives converging
      and goes through its own 0.094 mm waist 0.14 m after P0, before Q1. What
      to watch is the middle -- a hard waist there is where a 1 nC beam loses
      its emittance even at 40 MeV, and it is why the choice is made on figures
      rather than by a solver.
...
settings block. The SC values are active by default. With `SC = True`, the
tracked P1 distribution is written to `outputs/OP2/beams/p1.ast` for the
compressor stage.
...
KI = -22.0                      # OP2 design of record; sets the target Twiss
...
# T1_G = (-2.6528, +2.0955, -0.8369)
# T2_G = (-3.2983, +4.3466, -5.5859)
...
def main():
    ...
    apply_style()
    save(figure_e2e(match, tws, SC), FIGS, 'fig_OP2_two_triplet_e2e')
    plt.show()
