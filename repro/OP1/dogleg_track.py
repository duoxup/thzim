#!/usr/bin/env python3
r"""OP1 dogleg: track the matched P1 beam through the compressor to P2.

    P1   d_in  B1(+t)  d1  QO  d2  QI  d3 | C | d3  QI  d2  QO  d1  B2(-t)  d_out   P2

The lattice is the OP1 design of record: the `DoglegGeom` and `ki` below are
...
Two tracks are run on the same beam and drawn together:
  ...
  * the COLLECTIVE track -- SC + CSR on. The difference is what the charge
    ...
    1 nC and 15 MeV that is the dominant physics of this section.
...
Input:  `outputs/OP1/beams/p1.ast` (from `two_triplet_e2e.py`, SC on)
Output: `outputs/OP1/beams/p2.ast`, consumed by the final match.
...
# OP1 design of record; must match two_triplet_e2e.py, whose entrance target
# was computed from this geometry and ki.
DOGLEG = DoglegGeom(theta_deg=40.0, rho=0.542, delta_x=2.0, d_in=0.2, d_out=0.1)
KI = -38.0                      # inner-pair k1 [1/m^2]; ko follows from the achromat
