#!/usr/bin/env python3
r"""OP1 M3 quadruplet match: dogleg exit (P2) -> undulator entrance (P3).

    P2   d_in   Q1   Q2   Q3   Q4   d_out   P3

The SASE branch's final focus: the four-quad M3 station after the dogleg,
solved on the transfer matrix by `thzim.quadruplet` exactly as the
superradiant branch's M1 is. Four knobs against the four exit Twiss numbers
is a square problem, so there is no scan and nothing for a person to choose.
...
(3.38, 0, 0.007, 0), a vertical waist of 7 mm at the undulator entrance. There
is no derivation script for it. Note what that asks of the beam: at 16 MeV
with the 1.8 % energy spread the dogleg leaves, a 7 mm waist 0.4 m past Q4 is
strongly chromatic -- the focal length varies by the energy spread and the
PROJECTED beta at the waist cannot reach the target even when the linear
optics is exact. The Bmag printed is on the projected Twiss, as in the design
of record, so it reports that chromatic residual honestly.
...
under space charge (~30 mm at 1 nC); the dispersion-corrected values are
printed next to it so the difference can be seen.
...
P2_DIST = output_dir(REPO, "OP1", "beams") / "p2.ast"
P3_DIST = output_dir(REPO, "OP1", "beams") / "p3.ast"
FIGS = output_dir(REPO, "OP1", "figures")

# P3 target (beta_x, alpha_x, beta_y, alpha_y): the FEL-side choice of record.
TARGET = (3.38, 0.0, 0.007, 0.0)

# M3: the four-quad station after the dogleg. d_in is the OP1 allocation of
# the interface drift (the dogleg's d_out = 0.1 m sits upstream of P2).
GEOM = QuadrupletGeom(lq=0.10, d_in=0.10, d_inter=0.30, d_out=0.40)
...
def figure_e2e(match, tws, sc):
    ...
    ax_l.set_xlabel(r"$s$ [$m$]   (P2 $\to$ Q1 Q2 Q3 Q4 $\to$ P3)")
    return fig
...
def main():
    ...
    apply_style()
    save(figure_e2e(match, tws, SC), FIGS, 'fig_OP1_m3_quadruplet_match')
    plt.show()
