#!/usr/bin/env python3
r"""OP4 chicane — the horizontal entrance beta: a tracking scan.
...
for OP4. The second figure puts the three candidate exit objectives (eps_n,x,
...
The first figure shows, per entrance beta_x:
  ...
  (d) sigma_z(s), against the same run with SC and CSR switched off -- how much
      ...
      |R51| sigma_x,beta of path-length spread and then cancels EXACTLY at the
      exit, because the full chicane has R51 = R52 = 0 by symmetry. Here
      R51 = -0.3357 after bend 3, so at beta_x = 60 m (sigma_x,beta = 1.53 mm)
      it predicts 0.51 mm of excess against the 0.53 mm the run shows; over the
      scan the excess in quadrature runs 0.15 / 0.25 / 0.53 mm at
      beta_x = 1 / 10 / 60 m. Strip the collective effects and every beta_x
      reaches the same exit sigma_z, 0.13032 mm. What is left is the real
      effect, and at OP4 it is not small: SC and CSR spoil the compression by
      52 % at beta_x = 1 m and still 31 % at 60 m, so a larger beam bunches
      substantially better too. Compare OP3, where the same column reads 8 % and
  ...
  (c) eps_n,x(s) dispersion-corrected, with eps_n,y for comparison -- where
      ...
      deliberately not drawn: inside the chicane eta reaches ~0.34 m and, with
      sigma_delta ~0.48 %, the dispersive term takes it to 36-76 um and back
      again, roughly 10x the exit value. That is bookkeeping, not growth. Only
      the exit value means anything, and the table prints it.
...
equivalent x-drift length, ~2.19 m here) -- and that is the WORST choice for
...
OP4 is the collective-dominated end of this branch, and the reason it is worth
re-running rather than reading off OP3. Three times the charge at roughly half
the energy (0.6 nC, 21.8 MeV against OP3's 0.2 nC, 39.9 MeV) is a far larger
space-charge parameter, and it shows in every column: the exit eps_n,x grows
from 1.69 um at the entrance to 6.8-13.3 um (OP3: to 2.6-4.1), the residual
dispersion leaks 93-112 mm rather than 12-14, and the compression loses 31-52 %
to collective effects rather than 5-8. The trade-off has the same SHAPE, so the
same beta_x wins, but here it is worth several um of emittance instead of one.

Input: `data/OP4_50k.dist`, conditioned per scan point with
...
depositing/interpolating the particles, not by the FFT, so 63^3 is only ~30 %
slower than 31^3. Measured convergence at beta_x = 30 m with SC+CSR, 50k
particles:

    mesh       t/s    enx proj    enx corr     eny     exit eta   peak sig_x
    15x15x15   11.4    7.2810 um   6.5935 um   1.7546   93.78 mm   1.8385 mm
    31x31x31   12.0    7.4668      6.6215      1.7713  100.74      1.8396
    63x63x63   15.6    7.6287      6.7527      1.7750  102.28      1.8399

Same picture as OP3: beam SIZE is converged already at 15^3 (peak sigma_x agrees
to 4 digits); the emittance and the residual dispersion are not, and still creep
~2 % from 31^3 to 63^3. Repeat the convergence check if the mesh or tracking
model changes.
...
DIST = REPO / "data" / "OP4_50k.dist"
FIGS = output_dir(REPO, "OP4", "figures")

GEOM = ChicaneGeom(theta_deg=19.05, l_bz=0.10, l_dz=0.75, lead=0.20)
EKIN_MEV = 21.8                       # OP4
...
def condition(dist, beta_x, alpha_x, beta_y, alpha_y, workdir):
    ...
    path = Path(workdir) / f"op4_bx{beta_x:g}.ast"
...
def main():
    ...
    plt.show()
    save(fig, FIGS, 'fig_OP4_chicane_beta_x_scan')
    save(fig2, FIGS, 'fig_OP4_chicane_beta_x_objectives')
